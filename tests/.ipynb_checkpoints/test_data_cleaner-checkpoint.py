...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        # - Llamar a drop_invalid_rows
        result_df = cleaner.drop_invalid_rows(df_original, ["name", "age"])
        
        # - Verificar que el DataFrame resultante tiene menos filas que el original 
        self.assertLess(len(result_df), len(df_original))
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # - Crear un DataFrame con espacios en blanco usando make_sample_df()
        df_original = make_sample_df()
        df_copy = df_original.copy() # Hacer una copia para verificar la inmutabilidad 
        cleaner = DataCleaner()
        
        # - Llamar a trim_strings
        result_df = cleaner.trim_strings(df_original, ["name"])
        pdt.assert_frame_equal(df_original, df_copy)
        
        # - Verificar que las columnas no especificadas (ej: "city")
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        # - Llamar a remove_outliers_iqr con la columna "age" y factor=1.5
        result_df = cleaner.remove_outliers_iqr(df_original, "age", factor=1.5)
        
        # - Verificar que el valor extremo (120) fue eliminado
        self.assertNotIn(120, result_df["age"].values)
        
        # Verificar que los valores no extremos 
